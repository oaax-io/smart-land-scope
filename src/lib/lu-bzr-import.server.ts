/**
 * Server-only LU BZR import (URLs → storage + regulation_documents).
 * Callable without a user session (e.g. from background jobs / cron).
 */
...
export async function importLuBzrDocumentsInternal(uploaderId?: string | null) {
  ...
  const results: Array<{ gemeinde: string; status: string; error?: string }> = [];
  ...
  for (const src of LU_BZR_SOURCES) {
    try {
      const { data: muni } = await supabaseAdmin
        ...
        .maybeSingle();
      if (!muni) { results.push({ gemeinde: src.gemeinde, status: "no_municipality" }); continue; }
      ...
      const { data: existing } = await supabaseAdmin
        ...
        .eq("active", true)
        .maybeSingle();
      if (existing) { results.push({ gemeinde: src.gemeinde, status: "already_exists" }); continue; }

      const resp = await fetch(src.url).catch((e) => {
        results.push({ gemeinde: src.gemeinde, status: "fetch_failed", error: String(e) });
        return null;
      });
      if (!resp) continue;
      if (!resp.ok) { results.push({ gemeinde: src.gemeinde, status: `http_${resp.status}` }); continue; }
