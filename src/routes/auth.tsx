...
export const Route = createFileRoute("/auth")({
  head: () => ({
    meta: [
      { title: "Anmelden — SmarTerra" },
      { name: "description", content: "Bei SmarTerra anmelden oder kostenlos registrieren." },
    ],
  }),
  component: AuthPage,
});
...
const passwordSchema = z.string().min(8, "Mindestens 8 Zeichen").max(128);
const nameSchema = z.string().trim().min(2, "Mindestens 2 Zeichen").max(80);
...
function AuthPage() {
  ...
  async function handleSignup(e: React.FormEvent<HTMLFormElement>) {
    ...
    setLoading(false);
    if (error) return toast.error(error.message);
    toast.success("Account erstellt. Sie können jetzt loslegen.");
    navigate({ to: "/dashboard" });
  }
  ...
  async function handleGoogle() {
    ...
    if (result.error) {
      setLoading(false);
      return toast.error("Google-Anmeldung fehlgeschlagen.");
    }
  ...
  async function handleReset() {
    const email = prompt("E-Mail-Adresse für den Reset:");
    if (!email) return;
    ...
    if (error) return toast.error(error.message);
    toast.success("Reset-Link versendet. Bitte E-Mails prüfen.");
  }
        ...
        <div className="w-full max-w-md">
          ...
          <Tabs defaultValue="login">
            ...
            <TabsContent value="login" className="mt-6">
              <h1 className="font-display text-2xl font-bold">Willkommen zurück</h1>
              <p className="mt-1 text-sm text-muted-foreground">Melden Sie sich an, um fortzufahren.</p>
            ...
            <TabsContent value="signup" className="mt-6">
              <h1 className="font-display text-2xl font-bold">Account erstellen</h1>
              <p className="mt-1 text-sm text-muted-foreground">14 Tage gratis. Keine Kreditkarte nötig.</p>
          ...
          <div className="my-6 flex items-center gap-3">
            ...
            <span className="text-xs uppercase tracking-wider text-muted-foreground">oder</span>
            <div className="h-px flex-1 bg-border" />
          </div>
